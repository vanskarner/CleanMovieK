...
class FindUpcomingMovieUseCaseTest {
    private lateinit var useCase: FindUpcomingMovieUseCase
    private val remoteRepository: MovieRemoteRepository = mock()

    @Before
    fun setup() {
        useCase = FindUpcomingMovieUseCase(remoteRepository)
    }

    @Test
    fun execute_withValidID_returnItem() = runTest {
        val expectedItem = MovieDetailDS(12, "", "", "", 12, 12f, "", "", false)
        `when`(remoteRepository.getMovie(expectedItem.id)).thenReturn(Result.success(expectedItem))
